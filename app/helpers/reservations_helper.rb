module ReservationsHelper
  def days_until_next_reservation
    ...
    next_day_one_of_my_boards_is_reserved = UnavailableDate.find(:first, :joins => "inner join reservations on parent_id = reservations.id inner join boards on reservations.board_id = boards.id", :order => "date", :conditions => ["date > ? AND reservations.deleted_at is null AND reservations.creator_id != ? AND boards.creator_id =?", Date.today, current_user.id, current_user.id])
