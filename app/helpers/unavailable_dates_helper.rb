module UnavailableDatesHelper
  ...
  def days_until_next_reservation
    # next_day_i_reserve_someone_elses_board = UnavailableDate.find(:first, :joins => :parent, :order => "date", :conditions => ["date > ? AND unavailable_dates.creator_id = ? AND boards.creator_id != ?", Date.today, current_user.id, current_user.id])
    #     next_day_one_of_my_boards_is_reserved = UnavailableDate.find(:first, :joins => :parent, :order => "date", :conditions => ["date > ? AND unavailable_dates.creator_id != ? AND boards.creator_id =?", Date.today, current_user.id, current_user.id])
    # 
